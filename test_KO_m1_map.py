...
parser.add_argument('--m', type=int, default=2, help='reed muller code parameter m')
...
parser.add_argument('--enc_train_snr', type=float, default=10., help='snr at enc are trained')
...
parser.add_argument('--model_iters', type=int, default=1000, help='model iterations')
...
# Binary_Binary_Dot_Matrices = {}
# Binary_Binary_Dot_Matrices[m] = torch.load('./data/{0}/Binary_Binary_Dot_Matrix.pt'.format(m + 1)).to(device)
...
def decoder_affine(corrupted_codewords, a,b, snr):
    ...
    L_1 = 2*k_a*k_b/noise_sigma**2*corrupted_codewords[:,0]
    L_2 = 2*k_a*k_b*a/noise_sigma**2*corrupted_codewords[:,1]
    L_3 = 2*k_a*k_b*b/noise_sigma**2*corrupted_codewords[:,2]
    L_4 = 2*k_a*k_b*a*b/noise_sigma**2*corrupted_codewords[:,3]

    L_v_2 = log_sum_exp(torch.stack([L_1, L_3], dim=1))+log_sum_exp(torch.stack([L_2, L_4], dim=1))
    v_2_hat = torch.tanh(0.5*L_v_2)
    
    L_v_1 = log_sum_exp(torch.stack([L_1+v_2_hat*L_3, L_2+v_2_hat*L_4], dim=1))
    v_1_hat = torch.tanh(0.5*L_v_1)
    
    L_u_1 = L_1+v_2_hat*L_3+v_1_hat*(L_2+v_2_hat*L_4)
    u_1_hat = torch.tanh(0.5*L_u_1)
    return torch.stack([u_1_hat, v_1_hat, v_2_hat], dim=1)
...
#         self.bn = nn.BatchNorm1d(64)
    ...
    # For the g2 encoder, these statistics are important for inference.
    def reset_pre_normstats(self, power_constraint_type): # Power constraint is either 'soft_power_bit' or 'soft_power_block'
                 
        self.mean_scalar = torch.zeros(4).type(torch.FloatTensor).to(self.this_device) if power_constraint_type == 'soft_power_bit'                                         else torch.zeros(1).type(torch.FloatTensor).to(self.this_device)
        
        self.std_scalar  = torch.ones(4).type(torch.FloatTensor).to(self.this_device) if power_constraint_type == 'soft_power_bit'                                         else torch.ones(1).type(torch.FloatTensor).to(self.this_device)

        
        self.num_test_block= 0.0
        
    def update_normstats_for_test(self, this_mean, this_std):
...
def encoder_full(msg_bits, gnet_dict, power_constraint_type, training_mode='train'):    #g_avector, g_bvector, 
...
def decoder_dumer_soft(corrupted_codewords, snr):
    ...
    decoded_bits = torch.zeros(corrupted_codewords.shape[0], m+1).to(device)
...
results_load_path = 'Results/RM({0},1)_softmap/NN_EncFull_Skip+Dec_Dumer/Enc_snr_{1}/Batch_{2}'\
...
def test_MAP_and_all(msg_bits, snr):
    ...
    dumer_decoded_bits = decoder_soft_FHT(corrupted_codewords_Plotkin, snr, args.m)[:, tree_bits_order_from_standard].sign() #_dumer    
...
ok = 1
plt.semilogy(snr_range[:-ok], bers_dumer_test[:-ok], label="RM + Soft FHT", marker='o', linewidth=1.5)
plt.semilogy(snr_range[:-ok], bers_nn_test[:-ok], label="Neural RM + Neural Soft MAP", marker='^', linewidth=1.5)
...
plt.title("BER plot of Neural RM({0},1) codes: Trained at Enc:{1}dB".format(args.m, args.enc_train_snr))
...
ok = 1
plt.semilogy(snr_range[:-ok], blers_dumer_test[:-ok], label="RM + Soft FHT", marker='o', linewidth=1.5)
plt.semilogy(snr_range[:-ok], blers_nn_test[:-ok], label="Neural RM + Neural Soft MAP", marker='^', linewidth=1.5)
...
# plt.title("Trained at Enc_SNR = {0} dB and Dec_SNR = {1} dB".format(enc_train_snr, dec_train_snr))
plt.title("BLER plot of Neural RM({0},1) codes: Trained at Enc:{1}dB".format(args.m, args.enc_train_snr))
plt.legend(prop={'size': 15})
